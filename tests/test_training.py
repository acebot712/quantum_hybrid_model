...
class TestTraining(unittest.TestCase):
    ...
    def test_train_model(self):
        losses, accuracies = train_model(self.model, self.X_train, self.y_train, n_epochs=10, plot_dir=self.plot_dir)
        self.assertEqual(len(losses), 10)
        self.assertEqual(len(accuracies), 10)

    def test_evaluate_model(self):
        accuracy = evaluate_model(self.model, self.X_test, self.y_test, plot_dir=self.plot_dir)
